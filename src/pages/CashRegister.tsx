...
// ✅ NEW: reverse mapping for rendering UI from DB values we fetch
...
export default function CashRegister() {
  ...
  // ✅ FIX: get the string timezone from the hook (not the whole object)
  const { timezone: businessTimezone } = useBusinessTimezone(); // ✅ (replaces previous incorrect usage)
  ...
  // ✅ NEW: dynamic payment methods (fallback to hardcoded if none found)
  const [paymentMethodsUI, setPaymentMethodsUI] = useState<UiPaymentMethod[] | null>(null);
  ...
  // -------- ✅ Fetch available payment methods from transactions (DB → UI) --------
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!businessId) {
        setPaymentMethodsUI(null);
        return;
      }
      const { data, error } = await supabase
        .from("transactions")
      ...
      if (error) {
        ...
        if (!cancelled) setPaymentMethodsUI(null);
        return;
      }

      const uniqueDb: DbPaymentMethod[] = Array.from(
        new Set(
          (data || [])
            .map((r: any) => r.payment_method)
            .filter((v: any): v is DbPaymentMethod =>
              v === "Cash" || v === "POS" || v === "Satispay" || v === "Other"
            )
        )
      );

      const uiList = uniqueDb.map((db) => DB_TO_UI_PAYMENT[db]);
      if (!cancelled) setPaymentMethodsUI(uiList.length ? uiList : null);
    })();
  ...
  // ---------- UI ----------
  const PAYMENT_METHODS_UI: UiPaymentMethod[] = ["Contanti", "POS", "Satispay", "Altro"]; // fallback only
              ...
              <div className="flex-1 p-6 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:outline_none focus:ring-2 focus:ring-black focus:border-transparent text-black bg-white"
                  >
                    {(paymentMethodsUI ?? PAYMENT_METHODS_UI).map((m) => (
                      <option key={m} value={m as UiPaymentMethod}>
                        {m}
                      </option>
                    ))}
