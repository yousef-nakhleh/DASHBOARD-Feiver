...
export function PhoneCallerGate({ children, fallback = null }: Props) {
  const { ready, has } = useFeatures();

  if (!ready) return null; // wait until features are loaded
  return has(FEATURE.PHONECALLER) ? <>{children}</> : <>{fallback}</>;
}

export default PhoneCallerGate;